...
// ── Debug: force the interaction/QR state with dummy values ──
// `?debug=interaction` — shows the protocol log up to and including the
// "Interaction Required" step, with a QR, code, and link. No polling.
// `?debug=approved` — extends the interaction flow with a simulated
// "Authorization Granted" step (QR scanned + approved on the PS).
const __debugState = new URLSearchParams(location.search).get('debug')
if (__debugState === 'interaction' || __debugState === 'approved') {
  ...
  showLog()
  addLogStep('POST /authorize', 'success',
    formatRequest('POST', '/authorize', { 'Content-Type': 'application/json' }, { scope: 'openid profile', ps: dummyPs }) +
    '<label style="margin-top: 0.5rem;">Response</label>' +
    formatResponse(200, null, { ps_metadata: dummyMeta })
  )
  addLogStep('Discover Person Server', 'success',
    formatRequest('GET', `${dummyPs}/.well-known/aauth-resource.json`, null, null) +
    '<label style="margin-top: 0.5rem;">Response</label>' +
    formatResponse(200, null, dummyMeta)
  )
  addLogStep('Resource Token Created', 'success',
    formatToken('Resource Token (aa-resource+jwt)', dummyResourceToken, dummyDecoded)
  )
  addLogStep('POST /token', 'success',
    formatRequest('POST', dummyMeta.token_endpoint, { 'Content-Type': 'application/json' }, { resource_token: dummyResourceToken }) +
    '<label style="margin-top: 0.5rem;">Response</label>' +
    formatResponse(202, { 'aauth-requirement': `interaction; code="DEMO1234"` }, { requirement: 'interaction', code: 'DEMO1234' })
  )
  const interactionStep = addLogStep('Interaction Required', 'pending',
    renderInteraction(
  ...
  if (__debugState === 'approved') {
    ...
    addLogStep('Authorization Granted', 'success',
      formatResponse(200, null, { auth_token: dummyAuthToken }) +
      formatToken('Auth Token', dummyAuthToken, dummyAuthDecoded)
    )
  }
}
